...
void drawKeyboard(uint8_t xOffset, uint8_t yOffset) {
    ...
    // constexpr uint8_t key_PosX[] = { 1, 5, 3, 3, 3, 4, 5, 6, 8, 7, 8, 9, 7, 6, 9, 10, 1, 4, 2, 5, 7, 4, 2, 2, 6, 1 }; 
    ...
    if (xCursor == 0 && yCursor == 2) {
        FX::drawBitmap(xOffset, 2 * (key_Height - 1) + yOffset, Images::Keyboard_W[26], 0, dbmReverse);
    }
    else {
    ...
    if (xCursor == 8 && yCursor == 2) {
        FX::drawBitmap(7 * (key_Width - 1) + (key_Width - 1) + xOffset + 2, 2 * (key_Height - 1) + yOffset, Images::Keyboard_W[27], 0, dbmReverse);
    }
    else {
